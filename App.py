...
def cleanup_temp_files():
  ...
  for f in files:
    try:
      ...
      pass
  st.session_state['audio_path'] = None
  if 'yt_error' in st.session:
      del st.session_state['yt_error'] 

def download_youtube_audio(url):
    """Download only audio from Youtube using relaible browser impersonation."""
    audio_opts = {
...
def handle_youtube_download(url):
    """Callback function to ensure session state persist after button click"""
    try:
